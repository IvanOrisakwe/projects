...
from alkukantaiset_objektit import Luola
from otukset import Hahmo, Bugi
...
class Peli:
    """Game logic; keeps its own private state and uses methods only."""
    ...
    # ---- drawing helpers using prompt API ----
...
def game_loop(koko=10, bugeja=6):
    ...
    while True:
        ...
        key = read()
        if key in (0, 224):  # arrow key prefix (Windows consoles)
            key = read()
        if key == 27:  # ESC
            break
        peli.tick(key)
